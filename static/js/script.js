...
function setMode(mode) {
    ...
    if (mode === "report") {
        ...
        document.getElementById("panel-optimization")?.classList.remove("hidden");
    } else if (mode === "users") {
...
function updateCharts() {
    ...
    if (equipmentChart && state.summary) {
        const busy = state.summary.busy_units || 0;
        const total = (state.units && state.units.length) || 0;
...
function toggleMapView() {
    ...
    // Cycle logic: Standard -> Collapsed -> Expanded -> Standard
    if (!mapElement.classList.contains('collapsed') && !mapElement.classList.contains('expanded')) {
        mapElement.classList.add('collapsed');
        if (viewBtn) viewBtn.innerText = '🔼';
        if (handleText) handleText.innerText = 'EXPAND DATA DRAWER';
        showToast("View Mode", "Data Focus: Panels Maximized", "info");
    } else if (mapElement.classList.contains('collapsed')) {
        mapElement.classList.remove('collapsed');
        mapElement.classList.add('expanded');
        if (viewBtn) viewBtn.innerText = '🔽';
        if (handleText) handleText.innerText = 'EXPAND TACTICAL MAP';
        showToast("View Mode", "Tactical Focus: Map Maximized", "info");
    } else {
        mapElement.classList.remove('expanded');
        if (viewBtn) viewBtn.innerText = '↕️';
        if (handleText) handleText.innerText = 'RESTORE BALANCED VIEW';
...
async function runOptimizationEngine(algoType) {
    ...
    const algoMap = {
        ...
        'nsga':   { label: 'NSGA-II Evolutionary',         endpoint: '/api/optimize',     method: 'nsga' },
        'hybrid': { label: 'Hybrid PSO-GWO',               endpoint: '/api/optimize',     method: 'hybrid' }
    };
    ...
    if (planDiv) planDiv.innerText = `⚙️ Running ${algo.label}...\nPlease wait...`;
    ...
    let payload = {};
    if (algoType === 'ip') {
        payload = { budget: 10000, horizon: 300, dominoTime: 30, scenario: 1, costTruck: 300, costHeli: 800, costDrone: 100 };
    } else if (algoType === 'gp') {
        payload = { targetDamage: 400, targetCost: 4000, w1: 0.5, w2: 0.5, budget: 10000, horizon: 300 };
    } else {
        payload = { lat, lng, severity, algorithm: algo.method };
    }
    ...
    try {
        ...
        // Update plan details
        if (planDiv) planDiv.innerText = data.log || 'Optimization complete.';
...
async function loadUsers() {
    ...
    try {
        ...
        if (!users || users.length === 0) {
            grid.innerHTML = `<div style="padding:30px; text-align:center; color:rgba(255,255,255,0.3);">
                <p>No users found in system.</p></div>`;
            return;
        }

        const roleColors = { admin: '#ef4444', fireman: '#f97316', citizen: '#22c55e' };
        const roleIcons  = { admin: '🛡️', fireman: '🚒', citizen: '👤' };

        grid.innerHTML = users.map(u => {
            const roleColor = roleColors[u.role] || '#a78bfa';
            ...
            " onmouseover="this.style.borderColor='rgba(167,139,250,0.4)'; this.style.transform='translateY(-2px)'"
               onmouseout="this.style.borderColor='rgba(255,255,255,0.08)'; this.style.transform='translateY(0)'">
                ...
                <div style="
                    ...
                    font-weight: 800; font-size: 1rem; color: ${roleColor};
                    flex-shrink: 0;
                ">${u.picture ? `<img src="${u.picture}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;" onerror="this.outerHTML='${initials}'">` : initials}</div>

                <div style="flex:1; min-width:0;">
                    <div style="font-weight: 700; font-size: 0.95rem; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        ${u.name || 'Unknown'}
                    </div>
                    <div style="font-size: 0.78rem; color: rgba(255,255,255,0.4); margin-top: 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                        ${u.email}
                    </div>
                ...
                <div style="flex-shrink:0; text-align: right;">
                    <div style="
                        ...
                        margin-bottom: 8px;
                    ">${roleIcon} ${(u.role || 'citizen').toUpperCase()}</div>
                    <div>
                        <select onchange="updateUserRole('${u.email}', this.value)" style="
                            background: rgba(0,0,0,0.3); border: 1px solid rgba(255,255,255,0.1);
                            color: #aaa; padding: 4px 8px; border-radius: 6px; font-size: 0.75rem;
